                ...
                <div className="relative z-20">
                  <TabsList aria-label={t.language || "Language"} className="grid-cols-9">
                    {languageOptions.map((option) => {
                      const flagImage = languageFlagImages[option.code];
                      ...
                      return (
                        ...
                        >
                          <img
                            src={`${process.env.PUBLIC_URL}/${flagImage}`}
                            alt=""
                            aria-hidden="true"
                            className="h-5 w-7 rounded-[0.25rem] border border-white/70 object-cover opacity-80 shadow-sm transition-all group-hover:opacity-100 group-data-[state=active]:scale-105 group-data-[state=active]:opacity-100 sm:h-6 sm:w-9"
                          />
                        </TabsTrigger>
                      );
                    })}
                          ...
                          >
                            <div className="flex items-start justify-between gap-4">
                              <div className="flex min-w-0 flex-1 items-start gap-3 text-lg font-semibold text-zinc-900">
                                <svg
                                  className="mt-1 h-5 w-5 flex-shrink-0 text-zinc-500"
                                  fill="none"
                                  stroke="currentColor"
                                  viewBox="0 0 24 24"
                                >
