...
export default function Navbar({ children }) {
  ...
  const handleLanguageChange = async (val) => {
    setLanguage(val);
    try {
      localStorage.setItem("shigachat_lang", val);
    } catch {}
    try {
      await updateUserLanguage(val, setUser, setToken);
    } catch {}
  };
  ...
  const onNotificationMove = (n) =>
    handleNotificationMove(n, navigate, token, () =>
      fetchNotifications({
        language,
        token,
        userId,
  ...
  const onGlobalNotificationMove = (n) =>
    handleGlobalNotificationMove(n, navigate, token, () =>
      fetchNotifications({
        language,
        token,
        userId,
  ...
  const loadThreads = useCallback(async () => {
    ...
    try {
      ...
      const serverThreads = (data.threads || []).map((th) => ({
        id: String(th.thread_id ?? th.id),
        title: th.title,
      }));
      const withOverrides = serverThreads.map((t) => ({
        ...t,
        title: threadTitleOverrides[String(t.id)] ?? t.title,
      }));
  ...
  const selectThread = (id) => {
    ...
    try {
      window.dispatchEvent(new CustomEvent("threadSelected", { detail: String(id) }));
    } catch {}
  ...
  const deleteThread = async (thread) => {
    ...
    try {
      const resp = await fetch(
        `${API_BASE_URL}/question/delete_thread/${encodeURIComponent(String(thread.id))}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      const reload = await fetch(`${API_BASE_URL}/question/get_user_threads`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (reload.ok) {
        const data2 = await reload.json();
        const serverThreads = (data2.threads || []).map((th) => ({
          id: String(th.thread_id ?? th.id),
          title: th.title,
        }));
        const withOverrides = serverThreads.map((ti) => ({
          ...ti,
          title: threadTitleOverrides[String(ti.id)] ?? ti.title,
        }));
        setThreads(withOverrides);
      }
    } catch (e) {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden">
      ...
      <Sidebar
        ...
        style={{ width: isDrawerOpen ? "18rem" : "3.5rem", transition: "width 300ms ease" }}
      >
            ...
            <nav className="mb-6 space-y-1">
              <Tooltip content={t.tooltipHome} isVisible={!isDrawerOpen}>
                <div className={`${isDrawerOpen ? "" : "flex justify-center items-center"}`}>
                  <Link
                    title={!isDrawerOpen ? t.tooltipHome : undefined}
                    className={`flex items-center rounded text-sm text-zinc-900 transition-all duration-200 hover:bg-blue-50 hover:shadow-sm hover:scale-[1.02] ${isDrawerOpen ? "gap-3 px-3 py-2" : "justify-center items-center px-1 py-3"}`}
                    to="/home"
                  >
                    <Home
                      className={`text-blue-600 transition-all duration-200 hover:text-blue-700 hover:scale-110 ${isDrawerOpen ? "h-6 w-6" : "h-5 w-5"}`}
                    />
                    <span
                      className={
                        isDrawerOpen
                          ? "inline text-zinc-900 transition-colors duration-200 hover:text-blue-700"
                          : "hidden"
                      }
                    >
                      {t.home || "ホーム"}
              ...
              <Tooltip content={t.tooltipKeywordSearch} isVisible={!isDrawerOpen}>
                <div className={`${isDrawerOpen ? "" : "flex justify-center items-center"}`}>
                  <Link
                    title={!isDrawerOpen ? t.tooltipKeywordSearch : undefined}
                    className={`flex items-center rounded text-sm text-zinc-900 transition-all duration-200 hover:bg-blue-50 hover:shadow-sm hover:scale-[1.02] ${isDrawerOpen ? "gap-3 px-3 py-2" : "justify-center items-center px-1 py-3"}`}
                    to="/keyword"
                  >
                    <Search
              ...
              <Tooltip content={t.tooltipCategorySearch} isVisible={!isDrawerOpen}>
                <div className={`${isDrawerOpen ? "" : "flex justify-center items-center"}`}>
                  <Link
                    title={!isDrawerOpen ? t.tooltipCategorySearch : undefined}
                    className={`flex items-center rounded text-sm text-zinc-900 transition-all duration-200 hover:bg-blue-50 hover:shadow-sm hover:scale-[1.02] ${isDrawerOpen ? "gap-3 px-3 py-2" : "justify-center items-center px-1 py-3"}`}
                    to="/category"
                  >
                    <Layers
              ...
              <Tooltip content={t.tooltipQuestionManagement} isVisible={!isDrawerOpen}>
                <div className={`${isDrawerOpen ? "" : "flex justify-center items-center"}`}>
                  <Link
                    title={!isDrawerOpen ? t.tooltipQuestionManagement : undefined}
                    className={`flex items-center rounded text-sm text-zinc-900 transition-all duration-200 hover:bg-blue-50 hover:shadow-sm hover:scale-[1.02] ${isDrawerOpen ? "gap-3 px-3 py-2" : "justify-center items-center px-1 py-3"}`}
                  ...
                  {threads.map((th) => {
                    ...
                    return (
                      <li key={th.id} className="group relative">
                        <div
                          className={`flex items-center justify-between rounded px-2 py-1.5 text-sm transition-all duration-200 
                        ...
                        >
                          ...
                          {!isEditing && (
                            ...
                            >
                              <MoreHorizontal
                                className={`h-5 w-5 ${isMenuOpen || isActive ? "text-blue-700" : "text-zinc-500 hover:text-blue-700"}`}
                              />
                            </button>
                          )}
                        </div>
            ...
            >
              ...
              >
                <span className="relative inline-block">
                  <div
                    className={`flex items-center justify-center rounded-full bg-blue-600 font-semibold text-white transition-all duration-200 hover:bg-blue-700 hover:shadow-md hover:scale-110 ${isDrawerOpen ? "h-8 w-8 text-sm" : "h-8 w-8 text-base"}`}
                  >
                ...
                <div
                  className={
                    isDrawerOpen
                      ? "text-sm text-zinc-900 ml-2 transition-colors duration-200 hover:text-blue-700"
                      : "hidden"
                  }
                >
                            ...
                            >
                              <div className="mb-1 flex items-start justify-between">
                                <div className="flex-1 pr-2">
                                  <div
                                    className={`text-sm leading-relaxed ${n.is_read ? "text-zinc-700" : "text-zinc-900 font-medium"}`}
                                  >
                                    {n.message}
                                  </div>
                                </div>
                      ...
                      <div className="space-y-2">
                        {globalNotifications.length > 0 ? (
                          globalNotifications.map((n) => {
                            const isRead =
                              Array.isArray(n.read_users) && n.read_users.includes(userId);
                            return (
                              <div
                                key={n.id}
                                className={`group cursor-pointer rounded-lg border p-3 transition-all hover:border-blue-200 hover:shadow-sm active:bg-blue-100/60 [-webkit-tap-highlight-color:transparent] ${
                              ...
                              >
                                <div className="mb-1 flex items-start justify-between">
                                  <div className="flex-1 pr-2">
                                    <div
                                      className={`text-sm leading-relaxed ${isRead ? "text-zinc-700" : "text-zinc-900 font-medium"}`}
                                    >
                                      {n.message}
                                    </div>
                                  </div>
